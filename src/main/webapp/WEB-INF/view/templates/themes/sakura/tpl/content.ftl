...
<#--				<img src="${res_base_url!}/img/random/d-${randomMethod(1,10)}.jpg" onerror="imgError(this)"/>-->
		...
		<div class="p-time">
			...
			<i class="iconfont icon-time"></i><span>发布于 ${post.createTime?string('yyyy-MM-dd')}</span>
		</div>
		<p>${post.summary!}</p>
			...
			<div class="info-meta">
				...
				<div class="comnum">
					<span>
						...
						<a href="${post.fullPath!}#comments"><span>${post.commentCount!0} 条评论</span></a>
					</span>
				</div>
				</#if>
				<div class="views">
					<span><i class="iconfont icon-attention"></i><span>${post.visits?c} 热度</span></span>
				</div>
			</div>
		</footer><!-- .entry-footer -->
	</div>
