    ...
    <#if (settings.patternimg!true) && (settings.journals_patternimg?? && settings.journals_patternimg!='') || ((metas.ri?boolean)!true && settings.rimage_cover_sheet_open!true && settings.rimage_url?? && settings.rimage_url!='')>
        ...
        <div class="pattern-center">
            <div class="pattern-attachment-img">
                ...
                <img data-src="${res_base_url + settings.journals_patternimg!}" src="${res_base_url!}/img/svg/loader/orange.progress-bar-stripe-loader.svg"  class="lazyload" onerror="imgError(this)">
<#--                <#else>-->
<#--                    <img-->
    ...
    <#if journals.totalPages gt 1>
      <@paginationTag method="journals" page="${journals.number}" total="${journals.totalPages}" display="3">
        <center id="journals-pagination">
          ...
          <a href="${pagination.nextPageFullPath!}" class="at_button" style="margin-bottom: 15px;">加载更多...</a>
          </#if>
        </center>
      </@paginationTag>
    </#if>
    <#else>
        <span>博主太懒了，这里还什么都没有呢！！！</span>
    </#if>
</div>
<#include "footer.ftl">
<@footer />
