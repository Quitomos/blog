...
<#--                        srcset="<#if settings.rimage_cover_lqip == 'loading'>${res_base_url!}/img/svg/loader/orange.progress-bar-stripe-loader.svg<#else>${settings.rimage_url!}?tagid=tag&type=url&itype=${settings.rimage_cover_itype!}<#if settings.rimage_cover_itype != 'image'>&id=${(settings.rimage_cover_id)!''}</#if>&th=150</#if>"-->
        ...
        <div class="card">
            <div class="card-content">
                ...
                <div class="tag-title center-align">
                    <i class="fa fa-tags"></i>&nbsp;&nbsp;<span>文章标签</span>
                </div>
                </#if>
                <div class="tag-chips">
                <@tagTag method="list" contextPath="${theme_base!}">
                <#list tags as tag>
                <a href="${tag.fullPath!}" title="${tag.description!}">
