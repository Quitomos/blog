...
-->
<#include "header.ftl">
<@header title="${options.categories_title!'文章分类'} - ${blog_title!}">
    <#if (settings.patternimg!true) && (settings.category_patternimg?? && settings.category_patternimg!='') || ((metas.ri?boolean)!true && settings.rimage_cover_sheet_open!true && settings.rimage_url?? && settings.rimage_url!='')>
        ...
        <div class="pattern-center">
            <div class="pattern-attachment-img">
                ...
                <img data-src="${res_base_url + settings.category_patternimg!}" src="${res_base_url!}/img/svg/loader/orange.progress-bar-stripe-loader.svg"  class="lazyload" onerror="imgError(this)">
<#--                <#else>-->
<#--                    <img-->
...
<#--                        srcset="<#if settings.rimage_cover_lqip == 'loading'>${res_base_url!}/img/svg/loader/orange.progress-bar-stripe-loader.svg<#else>${settings.rimage_url!}?category=category&type=url&itype=${settings.rimage_cover_itype!}<#if settings.rimage_cover_itype != 'image'>&id=${(settings.rimage_cover_id)!''}</#if>&th=150</#if>"-->
        ...
        <div class="card">
            <div class="card-content">
                ...
                <div class="tag-title center-align">
                    <i class="fa fa-bookmark"></i>&nbsp;&nbsp;<span>文章分类</span>
                </div>
                </#if>
                <div class="tag-chips">
                ...
                <#list categories as category>
                    ...
                    <a href="${category.fullPath!}" title="${category.description!}" target="_self">
                    ...
                    <#if category.childList?has_content>
                        <br>
                        <#list category.childList as child>
                            <a href="${child.fullPath!}" title="${child.description!}" target="_self">
                                <span class="chip center-align waves-effect waves-light chip-default" data-tagname="${child.name!}">${child.name!}
                                    <span class="tag-length">${child.postCount!}</span>
                                </span>
                            </a>
                        </#list>
                    </#if>
