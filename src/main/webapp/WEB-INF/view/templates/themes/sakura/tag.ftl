...
<#--                        srcset="<#if settings.rimage_cover_lqip == 'loading'>${res_base_url!}/img/svg/loader/orange.progress-bar-stripe-loader.svg<#else>${settings.rimage_url!}?tagid=${tag.id}&type=url&itype=${settings.rimage_cover_itype!}<#if settings.rimage_cover_itype != 'image'>&id=${(settings.rimage_cover_id)!''}</#if>&th=150</#if>"-->
...
<#--                        class="lazyload<#if settings.rimage_cover_lqip == 'lowquality'> blur-up</#if>" />-->
            ...
            <header class="pattern-header">
                ...
                <span class="cat-des">${tag.description!}</span>
            </header>
        </div>
    <#else>
        <div class="blank"></div>
